...
struct HistoryTable<T: Value> {
     ...
     */
    func value(for path: Path, at date: Date?) throws -> Timestamped<T>? {
     ...
     */
    func optionalValue(for path: Path, at date: Date?) throws -> Timestamped<T?>? {
        let query = select(path: path, date: date)
